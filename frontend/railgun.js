...
let shoot = () => {
  log("Shooting");
  if (device) {
    log("Shooting railgun ...");
    log("Getting RailgunCommand Service ...");
    device.gatt.getPrimaryService('eac9cf2d-1d6e-4ab5-8582-bdc124b15e52')
    .then(service => {
      log('Getting Shooting Characteristic ...');
      return service.getCharacteristic('f17315d2-83e5-4f5e-9893-216ab8c5d9d6');
    })
    .then(characteristic => {
      log('Writing Shoot Characteristic ...');

      // Writing 1 is the signal to reset energy expended.
      let shootPercentage = Uint8Array.of(50);
      return characteristic.writeValue(shootPercentage);
    })
    .then(_ => {
      log('Railgun successfully shot the target!');
    })
    .catch(error => {
      log('ERROR ' + error);
    });
